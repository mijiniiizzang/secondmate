...
public class ChatService {
    ...
    // 거래 완료 처리
    @Transactional
    public void completeTrade(Long roomId, Long userId) {
        ChatRoom chatRoom = getAuthorizedRoom(roomId, userId);

        boolean isSeller = chatRoom.getProduct().getUser().getUserId().equals(userId);

        if (!isSeller) {
            throw new AccessDeniedException("판매자만 거래 완료 처리할 수 있습니다.");
        }
    ...
    private boolean canShowRoom(ChatRoom chatRoom, Long userId) {
        ...
        return chatMessageRepository.existsByChatRoom_RoomIdAndSender_UserIdNotAndSentAtAfter(chatRoom.getRoomId(), userId, leftAt);
    }
    ...
    private ChatRoomDTO toRoomDTO(ChatRoom chatRoom, Long loginUserId) {
        ...
        long unreadCount = chatMessageRepository.countByChatRoom_RoomIdAndSender_UserIdNotAndReadYn(chatRoom.getRoomId(), loginUserId, "N");
